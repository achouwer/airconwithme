"""Airconwithme climate entity (HA 2025 stable + crash-safe)."""
...
MODE_MAP_REV = {value: key for key, value in MODE_MAP.items()}
...
class AirconwithmeClimate(CoordinatorEntity, ClimateEntity):
    """HA 2025 compatible climate entity."""
    ...
    def available(self) -> bool:
        ...
        try:
            return bool(self.coordinator.data and self.coordinator.data.get("success"))
        except Exception:
            return False
    ...
    def hvac_mode(self) -> HVACMode:
        ...
        try:
            ...
            return MODE_MAP.get(raw, HVACMode.AUTO)
        except Exception:
    ...
    def current_temperature(self) -> float | None:
        ...
        return self._temp_from_raw(self._get(UID_ROOM_TEMP))

    @property
    def target_temperature(self) -> float | None:
        """Return target temperature."""
        return self._temp_from_raw(self._get(UID_TARGET_TEMP))

    @property
    def min_temp(self) -> float:
        """Return minimum target temperature."""
        return self._temp_from_raw(self._get(UID_MIN_TEMP)) or 18.0

    @property
    def max_temp(self) -> float:
        """Return maximum target temperature."""
        return self._temp_from_raw(self._get(UID_MAX_TEMP)) or 30.0
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        try:
            ...
            raw = MODE_MAP_REV.get(hvac_mode)
            if raw is None:
                return
    ...
    async def async_set_swing_mode(self, swing_mode: str) -> None:
        ...
        try:
            ...
            await self.coordinator.async_set_value(UID_VANE, raw)
        except Exception:
...
async def async_setup_entry(
    ...
    """Setup climate entity safely."""
    try:
        coordinator: AirconwithmeCoordinator = hass.data[DOMAIN][entry.entry_id][
            "coordinator"
        ]
        async_add_entities([AirconwithmeClimate(coordinator, entry)])
    except Exception:
        _LOGGER.exception("Climate setup failed")